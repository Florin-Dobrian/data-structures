...
Throughout both, `j` is the source column and `k` the target, with `j < k` always: column `j` updates column `k`.

**A — Left-looking (gather).** Column `k` waits until it is reached, then pulls in every earlier column `j`'s contribution in one pass, and only then divides its `L` part by the pivot. When the pass begins nothing to the right of column `k` has been touched; when it ends, column `k` is finished.

**B — Right-looking (scatter).** The moment column `j`'s multipliers are known they push a rank-1 update into every later column `k`. Column `j` finishes early and everything right of it is partially updated. That update is the outer product of column `j`'s multipliers with row `j` of `U`, which is where a real implementation calls a dense GEMM.

The two visit **the same set of `(j, k)` pairs**, and differ only in the nesting: left-looking runs `k` outer and `j < k` inner, right-looking runs `j` outer and `k > j` inner. With the loop variables named consistently the innermost statement comes out identical in both, which makes the relationship visible in the code rather than only in the description.

**Why it's interesting:** The same gather and scatter pair as the matvec, on an operation where it actually changes the shape of the program. In the matvec, gather and scatter differ only in loop order. Here they differ in *when a value becomes final*: left-looking finishes one column at a time and leaves the rest untouched, right-looking finishes a column immediately and leaves partial results everywhere. Since the `(j, k)` pairs are the same either way, the whole difference is *when* each update is applied, not which updates happen. Both compute the same `L` and `U`, and on this matrix they agree bitwise because every value is an exactly representable integer; on real input they would differ in the last bits, since the same contributions are summed in different orders.
