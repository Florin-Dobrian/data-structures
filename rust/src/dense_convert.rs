use crate::dense_matvec::{ColDenseMatrix, RowDenseMatrix};

/// Implementation A: row-major to column-major.
///
/// Same matrix, same m and n, same entries. Only the order they sit in the
/// buffer changes: row-major puts (i, j) at i * n_size + j, column-major puts
/// it at j * m_size + i. Every element moves, so this is a real copy rather
/// than a relabelling.
///
...
pub fn row_to_col_dense_convert(a: &RowDenseMatrix) -> ColDenseMatrix {
    let mut out = vec![0.0; a.val.len()];
    for i in 0..a.m_size {
        let rp = i * a.n_size; // row i begins here
        for j in 0..a.n_size {
...
pub fn col_to_row_dense_convert(a: &ColDenseMatrix) -> RowDenseMatrix {
    let mut out = vec![0.0; a.val.len()];
    for j in 0..a.n_size {
        let cp = j * a.m_size; // column j begins here
        for i in 0..a.m_size {
